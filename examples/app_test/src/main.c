...
int main(void)
{
    ...
    int fail = xtend_read(spiPort, blob, sizeof(blob));
    ...
    xtend_table_t tab;
    xtend_status_t irc = xtend_init(blob, &tab);
    ...
    const uint8_t *cp_base = tab.cp_base;
    const uint8_t *dp_base = tab.dp_base;

    printf("[app_test] exec_ram=%p magic=0x%08lX ver=%lu count=%lu\n",
           (void*)blob, tab.hdr->magic, tab.hdr->version, tab.hdr->count);

    printf("[app_test] cp=%p dp=%p cp_off=0x%08lX dp_off=0x%08lX\n",
           cp_base, dp_base, tab.hdr->cp_off, tab.hdr->dp_off);
