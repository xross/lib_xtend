...
#define XTND_IMAGE_MAX_SIZE (5000)
...
int simpleI2c(void)
{
    ...
    int fail = xtnd_read(spiPort, (uint8_t *) XTND_BASE_ADDR, XTND_IMAGE_MAX_SIZE);
    ...
    xtnd_fn_t fn_i2c_read = (xtnd_fn_t)xtnd_find(&tab, "xtnd_i2c_read");

    if (!fn_port_toggle_bit)
    {
        printf("[app_test] Failed to resolve xtnd function port_toggle_bit=%p\n", (void*)fn_port_toggle_bit);
        return 1;
    }

    if (!fn_i2c_read)
    {
        printf("[app_test] Failed to resolve xtnd function i2c_read=%p\n", (void*)fn_i2c_read);
        return 1;
    }

    int ret = xtnd_call(&tab, (void*)fn_i2c_read, 0x60, 0);
    printintln(ret);
